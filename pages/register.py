...
# ========== 先确保session_state保存邀请码（解决输入丢失问题） ==========
if "invite_number" not in st.session_state:
    st.session_state["invite_number"] = ""  # 用session_state保存邀请码
...
with st.form("login_form", clear_on_submit=False):
    ...
    invite_number = st.text_input("邀请码", type="password", placeholder="请输入您的邀请码", value="")
    submit_register = st.form_submit_button("注册", type="primary")

if submit_register:
    if username not in user_data:
        # 实时更新session_state（输入框内容变化时同步）
        st.session_state["invite_number"] = invite_number
        if not invite_number.strip():
            st.error("邀请码不能为空！")
        elif not username.strip() or not password.strip():  # 确保username/password可访问
            st.error("用户名和密码不能为空！")
            # 2. 邀请码正确性校验
        elif invite_number == "20060615":
            # 写入数据库（确保f.write_count_password方法参数正确）
            f.write_count_password(username, password)
            st.success("注册成功！")
            # 3. 重置状态（清空输入框+重置邀请码状态）
            st.session_state["invite_number"] = ""
